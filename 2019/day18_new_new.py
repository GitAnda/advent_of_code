...
class Tree:
    ...
    def make_matrix(self):
        self.distance_matrix.update(self.distances_to('@', self.initial_maze))
        for char in self.keys_and_doors:
            if char.islower():
                self.distance_matrix.update(self.distances_to(char, self.initial_maze))

        all_keys = [char for char in self.keys_and_doors if char.islower()]
        for key in all_keys:
            keys_from = [x for k, x in self.distance_matrix if k == key]
            while keys_from:
                for key_from in keys_from:
                    keys_to = [x for k, x in self.distance_matrix if k == key_from]
                    for key_to in keys_to:
                        dist_up, doors_up, _ = self.distance_matrix[key_from, key_to]
                        dist_down, doors_down, _ = self.distance_matrix[key, key_from]
                        self.distance_matrix[key, key_to] = (dist_up + dist_down, doors_up.union(doors_down), False)
            ...
            for i, item in enumerate(distances):
                ...
                for new_location in new_locations:
                    ...
                    if char == '.':
                        ...
                        distance_matrix[(key, char)] = (distance + 1, doors, True)
